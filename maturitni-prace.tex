...
\usepackage{MP}
\setlength{\parindent}{1em} % Nastaví odsazení prvního řádku odstavce na 15pt
...
\pagenumbering{roman}                   % číslování stránek římskými číslicemi
	...
	\podekovani{
		Rád bych poděkoval svému vedoucímu práce, Dr. rer. nat. Michalu Kočerovi, který mi byl oporou při psaní práce, ochotně se mnou konzultoval její průběh a jehož zásluhou jsem ji mohl vypracovat bez větších problémů. Dále bych chtěl poděkovat svému bývalému spolužákovi Šimonu Borovkovi, se kterým jsme tento projekt před několika lety společně začali a já jsem ho nyní měl možnost dokončit. Mé díky patří také mému otci, který byl jedním z prvních, kdo mou práci četl, a významně mi pomohl s její jazykovou korekturou. V neposlední řadě bych chtěl poděkovat Jonáši Havelkovi, jehož šablonu pro \LaTeX jsem v práci použil, a Václavu Baureovi a jeho maturitní práci na téma Systém Ball and Beam, která mi v mnoha případech posloužila jako výborný zdroj inspirace.
	}
...
V této práci se zabýváme drony, technologiemi spojenými s funkčností kvadrokoptér a konstrukcí kvadrokoptéry samotné. Hlavním účelem této práce je posloužit jako návodný materiál pro kohokoliv, kdo má zájem vyrobit si vlastní kvadrokoptéru v domácích podmínkách, a to i bez předchozích zkušeností. Práce poskytuje čtenáři základní vhled do celé problematiky a slouží jako odrazový bod pro budoucí konstrukční projekty.

V teoretické části se věnujeme definici dronu, popisu fungování jednotlivých komponent nezbytných pro jeho konstrukci a fyzice letu kvadrokoptér. V praktické části navrhneme a zkonstruujeme kvadrokoptéru. Popíšeme konkrétní použité součástky a zhodnotíme jejich výhody a nevýhody. Dále se zabýváme i vývojem softwaru pro ovládání kvadrokoptéry.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.55\linewidth]{battery.jpg}
...
Komunikace s HC-05 probíhá prostřednictvím sběrnice UART, která využívá asynchronní sériovou komunikaci pomocí dvou vodičů TX (transmit) a RX (receive) s definovanou přenosovou rychlostí (baud rate, např. 9600, 115200 baud). Na rozdíl od I2C zde chybí hodinový signál. UART nepodporuje adresaci více zařízení na jedné sběrnici, což znamená, že komunikace probíhá pouze point-to-point mezi dvěma zařízeními. \cite{uart}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\linewidth]{dron-blok.jpg}
